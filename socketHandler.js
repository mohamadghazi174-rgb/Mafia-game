...
// ═══════════════════════════════════════
const { getRoomByPlayer, deleteRoom } = require('../game/roomManager');
const { GameEngine }   = require('../game/gameEngine');
const { BotManager }   = require('../game/botManager');
const { checkRateLimit, clearSocket } = require('./middleware/rateLimiter');
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('auth_register', async ({ username, password, avatar = 1 }, cb) => {
      try {
        ...
        const { validateName, validatePassword } = require('./middleware/validator');
    ...
    socket.on('auth_guest', async ({ name }, cb) => {
      try {
        const userRepo = require('../db/repositories/userRepo');
        const { validateName } = require('./middleware/validator');
    ...
    socket.on('disconnect', (reason) => {
      ...
      const { GAME } = require('../server/config');
      setTimeout(() => {
